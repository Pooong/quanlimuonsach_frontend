    ...
    <div class="titlePage d-flex">
      ...
      <div class="ml-auto">
        <button v-if="isLogin" @click="showModal1" class="btnAdd" style="margin-right: 10px;">
          <i class="fa-solid fa-book-open" ></i> Thêm NXB
        </button>
        ...
        <form action="" enctype="multipart/form-data">
          <div class="contentModal row">
            <div class="rightModal col">
              ...
              <p class="errorText" v-if="errorsAddNXB.name">{{ errorsAddNXB.name }}</p>
              ...
              <p class="errorText" v-if="errorsAddNXB.diachi">{{ errorsAddNXB.diachi }}</p>
            </div>
          </div>
        </form>
      </a-modal>
...
<script setup>
  ...
  const errorsAddNXB = ref({
    name: "",
    diachi: ""
  })

  const handleValidateAddNXB = () => {
    console.log(tenNxb.value);
    console.log(diaChi.value);
    errorsAddNXB.value.name = tenNxb.value.trim() ? "" : "Vui lòng nhập tên NXB";
    errorsAddNXB.value.diachi = diaChi.value.trim() ? "" : "Vui lòng nhập Địa chỉ";
  }
  const handleValidate = () => {
    console.log("tenSach:", tenSach.value);
    console.log("idNxb:", idNxb.value);
  ...
  const handleOk1 = () => {
      ...
      if (isValid) {
        ...
        axios
        .post("http://localhost:3000/published",formData)
        .then((res) => {
          if (res.data.error) {
